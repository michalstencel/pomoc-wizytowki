        ...
        defineField({
            ...
            of: [
                ...
                defineArrayMember({
                    ...
                    title: "Tabela",
                    description:
                        "Wizualny edytor tabeli — wpisujesz tekst i linki w komórkach. Dla skomplikowanych tabel z formatowaniem użyj raczej bloku HTML poniżej.",
                    fields: [
                        defineField({
                            name: "hasHeader",
                            title: "Pierwszy wiersz to nagłówek",
                            type: "boolean",
                            initialValue: true,
                        }),
                        defineField({
                            ...
                            of: [
                                defineArrayMember({
                                    ...
                                    preview: {
                                        select: { cells: "cells" },
                                        prepare({ cells }) {
                                            const items = Array.isArray(cells)
                                                ? cells
                                                : [];
                                            const text = items
                                                .map(
                    ...
                    preview: {
                        select: { rows: "rows", hasHeader: "hasHeader" },
                        prepare({ rows, hasHeader }) {
                            const count = Array.isArray(rows) ? rows.length : 0;
                            return {
                                title: "Tabela",
                                subtitle: `${count} wierszy${hasHeader ? " (z nagłówkiem)" : ""}`,
                            };
                        },
                    },
                }),
